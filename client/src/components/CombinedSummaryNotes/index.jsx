  ...
  useEffect(() =>  {
    ...
    const unsubscribe = onSnapshot(docRef, async (docSnapshot) => {
      if (docSnapshot.exists() ) {
        ...
        const localStorageDataRaw = await localStorage.getItem('id');
        console.log(localStorageDataRaw,'rawr')
        const localStorageData = JSON.parse(localStorageDataRaw);
        console.log(localStorageData,'house');
        const token = await localStorage.getItem('token');
        console.log(token,'token');
        const tokenResponse = await fetch(`http://localhost:3000/token/get/${token}`);
        console.log(tokenResponse,'bowchikawow')
        const userData = await tokenResponse.json();
        console.log(userData, 'userData');
        const userEmail = userData.email;
        console.log(userEmail,'ola')
        const visibilityFromFirestore = data.visibility;
        console.log(visibilityFromFirestore,'monkey');
        const viewers = data.viewers;
        console.log(viewers, 'viewers')
        const viewersEmails = viewers.map(viewer => viewer.email);
        console.log(viewersEmails,'tootsie')
        
        if (localStorageData !== data.userID && visibilityFromFirestore === 'private' || !viewersEmails.includes(userEmail)) {
