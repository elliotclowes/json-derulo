...
const Controls = ({ handlers, isRecording }) => {
  // Local state to keep track of hover status
  const [isHovered, setIsHovered] = useState(false);

  const buttonWidth = '190px';
  ...
  <button 
    ...
    className="px-6 py-3.5 text-base font-medium text-white flex items-center justify-center bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 rounded-lg dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800"
  >
   <svg aria-hidden="true" role="status" className="inline w-4 h-4 mr-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">     
    ...
    {isHovered ? 'Stop recording' : 'Recording...'}
  </button>
) : (
  <button 
    type="button" 
